...
export dumbell_collapser,dumbell_collapser_min

function dumbell_collapser(i,Master_path,nudge_factor,err_marg)
    ...
    for mn_idx in mn_idxs
        ...
        if tet_charge(mn_tet,phi_arr,err_marg) !=1.0
            println("no more starting mono")
            println(tet_charge(mn_tet,phi_arr,err_marg))
            continue
        end
        
        if abs(round(del_plk(mn_tri,phi_arr)/(2*pi),digits=1))!=1.0
            println("no more starting string")
            continue
        end
        ...
        if res[1][1]==1
            ...
            uncmns = find_ucommons(mn_tri,amn_swap_ver)#vertices 3 & 4
            ...
            # println(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr))
            if abs(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr,err_marg))!=1.0
                println("antimonopole no longer there")
                # println(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr))
                continue
            end

            phase_234 = del_plk(mn_tri,phi_arr)/(2*pi)
            # println(phase_234);exit()
            if abs(round(phase_234,digits=2))!=1.0
                println("string no longer there")
                # println(phase_234)
                continue
            end

            # phi_arr=n_annihilate(ver_1,ver_2,ver_3,ver_4,ver_5,mn_tri,phi_arr,nudge_factor)
            phi_arr=n_annihilate_2(ver_1,ver_2,ver_3,ver_4,ver_5,mn_tri,phi_arr,
            nudge_factor,tet_locs,err_marg,mn_idx,res[2][1][1],N)
            # phi_arr=nhilate_swap(ver_1,ver_2,ver_3,ver_4,ver_5,mn_tri,phi_arr,
            # nudge_factor,tet_locs,err_marg,mn_idx,res[2][1][1],N)
            ...
            if (abs(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr,err_marg))==1.0 ||
                abs(tet_charge(mn_tet,phi_arr,err_marg))==1.0)
                println("monopole-antimonopole not removed.first move")
                println(abs(tet_charge(mn_tet,phi_arr,err_marg)))
                println(abs(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr,err_marg)))
                # phi_arr=n_annihilate(ver_1,ver_2,ver_3,ver_4,ver_5,mn_tri,phi_arr,nudge_factor)
                # exit()
                # exit()
                # break
        ...
        while res[1][1]!=1
            ...
            if res[1][1]==2
                ...
                if abs(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr,err_marg*10))!=1.0
                    println("monopole did not move")
                    println(tet_charge(tet_locs[res[2][1][1],:,:],phi_arr,err_marg*10))
                    break
                end
                mn_tri = next_tri
...
function dumbell_collapser_min(i,Master_path,nudge_factor)
