...
# @njit(parallel=True)
# @jit()
def derivatives(phi_arr_i, diff_arr):
    arr = phi_arr_i
...
# @njit(parallel=True)
def B_stack(diff_arr, stack, stack_mag,B_vec,interp):
                ...
                for z_idx in range(1,N-1,2):
                    ...
                    for j in range(3):
                        stack[x_idx,y_idx,z_idx,j] = np.mean([stack[x_idx+(1+bdxr),y_idx,z_idx,j]
                                                                ...
                                                                ,stack[x_idx,y_idx,z_idx-1,j]])
    ##Following are loops to remove nonsense values at unused points on the edges of the supercells##
    if interp==False:
        #Loops over the unused points on the edges parallel to x axis#
        for x_idx in range(1,N-1,2):
            for y_idx in range(0,N,2):
        ...
        for x_idx in range(0,N,2):
            for y_idx in range(0,N,2):
                for z_idx in range(1,N-1,2):
                    ...
                    for j in range(3):
                        stack[x_idx,y_idx,z_idx,j] = 0.0
    return np.real(stack), np.real(stack_mag)
