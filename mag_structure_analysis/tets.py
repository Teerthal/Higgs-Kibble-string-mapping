...
def tet_plot_jl(i,string_list,err_mar,xlims=[0,N],ylims=[0,N],zlims=[0,N]):
...
def tet_plot_slice_jl(i,string_list,err_mar,N,xlims=[0,N],ylims=[0,N],zlims=[0,N]):
    ...
    mono_idx = np.argwhere(np.logical_and(normed>= 1-err_mar, normed<=1 + err_mar))[:,0]
    antimon_idx = np.argwhere(np.logical_and(normed<= -1+err_mar, normed>=-1-err_mar))[:,0]
    # print(np.shape(mono_idx)[0])
    # print(np.mean(tet_locs[mono_idx,:,0],axis=1),np.mean(tet_locs[mono_idx,:,1],axis=1),np.mean(tet_locs[mono_idx,:,2],axis=1))
    
    x_monos = np.floor(np.mean(tet_locs[mono_idx,:,0],axis=1))
    y_monos = np.floor(np.mean(tet_locs[mono_idx,:,1],axis=1))
    z_monos = np.floor(np.mean(tet_locs[mono_idx,:,2],axis=1))
    x_amonos = np.floor(np.mean(tet_locs[antimon_idx,:,0],axis=1))
    y_amonos = np.floor(np.mean(tet_locs[antimon_idx,:,1],axis=1))
    z_amonos = np.floor(np.mean(tet_locs[antimon_idx,:,2],axis=1))
    for n in range(N):
        plt.figure()
        tar = np.argwhere(x_monos == n)
        tar_a = np.argwhere(x_amonos == n)
        if len(tar)>0:
            plt.scatter(y_monos[tar],z_monos[tar], marker='o',c='blue',linewidths=1)
            plt.xlim(ylims)
            plt.ylim(zlims)
        if len(tar_a)>0:
            plt.scatter(y_amonos[tar_a],z_amonos[tar_a], marker='o',c='red',linewidths=1)
            plt.xlim(ylims)
            plt.ylim(zlims)
        plt.title('x:%s'%n)
        plt.savefig('%s/%i_slice_%05i.png'%(Plot_path,n,i),dpi=600)
        # plt.show()
        plt.close()
